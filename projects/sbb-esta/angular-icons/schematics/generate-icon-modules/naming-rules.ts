...
const rules: Array<(svgFile: SvgFile) => void> = [
  ...
  file =>
    (file.modules = file.modules.filter(m => !invalidModules.includes(m))),
  file =>
  ...
  file =>
    (file.name = file.name
      ...
      .replace(/_/g, '-')),
  file =>
    (file.modules = file.modules.map(
      (v, i, a) => `${a.slice(0, i).join('')}${v}`
    ))
];
export const namingRules: Array<(svgFile: SvgFile) => SvgFile> = rules.map(
  r => (f: SvgFile) => {
    r(f);
    return f;
  }
