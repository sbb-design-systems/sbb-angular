  ...
  describe('standard calendar body', () => {
    ...
    it('places label in first row if space is available', () => {
      ...
      expect(rowEls[0].firstElementChild!.classList)
        .toContain('sbb-calendar-body-label', 'first cell should be the label');
      expect(labelEls[0].getAttribute('colspan')).toBe('3');
    });

    it('cell should be selected on click', () => {
      const todayElement =
        calendarBodyNativeElement.querySelector('.sbb-calendar-body-today') as HTMLElement;
      todayElement.click();
      fixture.detectChanges();

      expect(todayElement.classList)
        .toContain('sbb-calendar-body-selected', 'today should be selected');
    });
