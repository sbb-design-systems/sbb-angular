...
@Component({
  ...
  template: `<sbb-tooltip #t1 (opened)="onOpen($event)" (closed)="onClose($event)">
...
describe('TooltipComponent using mock component for single tooltip', () => {
  ...
  it('open tooltip by question mark', () => {
    ...
    expect(buttonQuestionMark.attributes['class'].value).toBe('sbb-tooltip-trigger sbb-tooltip-trigger-active');
  ...
  it('close tooltip by internal button', () => {
    ...
    const internalButton = singleFixtureTest.debugElement.query(By.css('.sbb-button-secondary')).nativeElement;
