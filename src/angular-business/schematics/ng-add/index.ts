...
export default function (options: Schema): Rule {
  return (host: Tree, context: SchematicContext) => {
    if (getPackageJsonDependency(host, '@sbb-esta/angular-public')) {
      context.logger.info(
        '@sbb-esta/angular-public is already installed, please set up @sbb-esta/angular-business manually'
      );
      return;
    }
    ...
    // The CLI inserts `@sbb-esta/angular-business` into the `package.json` before this schematic runs.
    // This means that we do not need to insert @sbb-esta/angular-business into `package.json` files again.
    // In some cases though, it could happen that this schematic runs outside of the CLI `ng add`
    // command, or @sbb-esta/angular-business is only listed a dev dependency. If that is the case, we insert a
    // version based on the current build version (substituted version placeholder).
    if (sbbAngularVersionRange === null) {
      addPackageToPackageJson(host, '@sbb-esta/angular-business', fallbackSbbAngularVersionRange);
    }
