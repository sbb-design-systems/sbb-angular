...
@Directive()
export class SbbSidebarContentBase extends CdkScrollable implements AfterContentInit {
  constructor(
    private _changeDetectorRef: ChangeDetectorRef,
    @Inject(forwardRef(() => SbbSidebarContainerBase)) public _container: SbbSidebarContainerBase,
 ...
 */
@Directive()
export class SbbSidebarBase implements AfterContentInit, AfterContentChecked, OnDestroy {
  ...
  constructor(
     ...
     */
    @Optional() @Inject(SBB_SIDEBAR_CONTAINER) public _container?: SbbSidebarContainerBase
  ) {
 ...
 */
@Directive()
export class SbbSidebarContainerBase implements AfterContentInit, DoCheck, OnDestroy {
  /** The sidebar child */
  get sidebar(): SbbSidebarBase | null {
  ...
  get hasBackdrop() {
    if (this._backdropOverride == null) {
      return !this._sidebar || this._sidebar.mode !== 'side';
    }
  ...
  @ContentChildren(SbbSidebarBase, {
    // We need to use `descendants: true`, because Ivy will no longer match
    // indirect descendants if it's left as false.
    descendants: true,
  })
  _allSidebars: QueryList<SbbSidebarBase>;

  /** Sidebars that belong to this container. */
  _sidebars = new QueryList<SbbSidebarBase>();

  @ContentChild(SbbSidebarContentBase) _content: SbbSidebarContentBase;
  @ViewChild(SbbSidebarContentBase) _userContent: SbbSidebarContentBase;
  ...
  /** The sidebar */
  private _sidebar: SbbSidebarBase | null;
  ...
  ngAfterContentInit() {
    this._allSidebars.changes
      ...
      .subscribe((sidebars: QueryList<SbbSidebarBase>) => {
        // @breaking-change 10.0.0 Remove `_container` check once container parameter is required.
        this._sidebars.reset(
          sidebars.filter((item) => !item._container || item._container === this)
        );
    ...
    this._sidebars.changes.pipe(startWith(null)).subscribe(() => {
      ...
      this._sidebars.forEach((sidebar: SbbSidebarBase) => {
   ...
   * is properly hidden.
   */
  private _watchSidebarToggle(sidebar: SbbSidebarBase): void {
  ...
  private _watchSidebarMode(sidebar: SbbSidebarBase): void {
    if (sidebar) {
  ...
  private _canHaveBackdrop(sidebar: SbbSidebarBase): boolean {
    return sidebar.mode !== 'side' || !!this._backdropOverride;
  }

  private _isSidebarOpen(sidebar: SbbSidebarBase | null): sidebar is SbbSidebarBase {
